...
opcodes = (
        ...
        ("1011 00100 0000 0000", "1111 0000 0000 0000", "call"),
...
def execute(ir) :
    ...
    if opcode == "halt":  
        ...
        while inp == "" \
          or  (not (inp.isdigit() \
            or (inp[0] == '-' and inp[1:].isdigit()))) \
          or not valid_integer(int(inp)):
            print("\n\nIllegal inp: number must be in [-32768,32767]")
            inp = input("Enter number (q to quit): ")
            if inp == "q" :
                sys.exit()
        register[args[0]] = int(inp) 
